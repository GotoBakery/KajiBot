...
- **完全データ管理**: 家事のメニュー、ポイント、労いメッセージ設定など、全て**スプレッドシート側で変更可能**。コード修正は不要です。
    - **新機能: 差分清算 (Reset)**: 「何でも言うこと聞く券」などのRESETタスクを実行すると、負けているユーザーに相手との差分（Gap）ポイントを加算し、強制的に同点（Gap 0）にします。これまでの積み上げ（TOTAL）は維持したまま、競争だけをリセットできます。
...
- **リッチなグラフ表現**:
    - ドーナツグラフ: 左右の配置がUIと一致（左がUser A、右がUser B）。
    - 中央にTOTALポイントを表示し、二人の積み上げを可視化。
- **UX改善**:
    - **日付のみやすい表示**: 「今日」「昨日」「M/d」を自動で切り替え。
    - **労いカテゴリ**: 「労い/Gift」カテゴリはゴールド（金色）の特別デザインで感謝を演出。
- **Undo機能**:
    - 誤って記録しても、直後のトースト通知から「取り消す」ことが可能。
    - 直近の記録を取り消すと、ポイントとDiscord通知も同期してリセットされます。
- **スクリーンセーバー (デジタルフォトフレーム)**:
    - **自動起動**: 5分間操作がないと自動で起動。
    - **癒やしのスライドショー**: 厳選された30枚の画像（動物、自然、カフェ、インテリア）をランダム（シャッフル）再生。
    - **スムーズな切り替え**: 画像プリロードとフェードアニメーションにより、途切れのない美しい表示を実現。
    - **時計表示**: 時間と日付をオーバーレイ表示し、フォトフレーム兼時計として機能します。
