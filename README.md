...
- **スクリーンセーバー (デジタルフォトフレーム)**:
    - **自動起動**: 5分間操作がないと自動で起動。
    - **癒やしのスライドショー**: 厳選された50枚の画像（動物、自然、カフェ、インテリア）をランダム（シャッフル）再生。
    - **スムーズな切り替え**: 画像プリロードとフェードアニメーションにより、途切れのない美しい表示を実現。
    - **時計表示**: 時間と日付をオーバーレイ表示し、フォトフレーム兼時計として機能します。
- **ミルク追跡 (Milk Tracker)**:
    - **専用モード**: 「ミルク」のタスクを記録する際、専用の数量入力モーダルが起動。
    - **スライドインダッシュボード**: 記録直後にダッシュボード右側から専用画面がスライドイン。
    - **可視化**: 直近の授乳履歴（経過時間の強調表示）と、日別トータル量を棒グラフで確認できます。
    - **目標ライン**: 1日の目標摂取量（点線）に対する進捗を一目で把握可能。
...
3. **スクリプトプロパティを設定**:
    ...
    - `DAILY_MILK_TARGET`: (任意) 1日のミルク目標量(ml)。デフォルトは `800`。
4. デプロイ（Webアプリとして導入）。
    - *アクセス権限: 全員 (Anyone)*
